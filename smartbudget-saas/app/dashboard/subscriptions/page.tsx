      ...
      <section className="card p-6">
        <form className="grid gap-4 md:grid-cols-[1fr_auto_auto] md:items-end">
          <div>
            ...
            <input name="q" defaultValue={q} className="input" placeholder="Nombre, categoría, estado o frecuencia" />
          </div>
      ...
      <section className="grid gap-6 xl:grid-cols-2">
        <div className="card p-6">
          <div className="mb-5 flex items-end justify-between gap-4">
            <div>
              <h2 className="text-xl font-semibold">Agregar suscripción</h2>
              <p className="text-sm text-slate-500">Lleva control de renovaciones automáticas.</p>
            </div>
            <div className="rounded-2xl bg-brand-50 px-4 py-3 text-sm">
              <span className="block text-xs uppercase tracking-wide text-brand-700">Costo mensual activo</span>
              <span className="text-lg font-semibold text-brand-700">{formatCurrency(activeMonthly)}</span>
            </div>
          </div>
        ...
        <div className="card p-6">
          <div className="mb-5">
            <h2 className="text-xl font-semibold">Agregar pago recurrente</h2>
            <p className="text-sm text-slate-500">Anota facturas y cargos fijos.</p>
          </div>
      ...
      <section className="grid gap-6 xl:grid-cols-2">
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <span className="badge">{subscriptions.length} resultados</span>
          </div>
          {subscriptions.map((item) => (
            <div key={item.id} className="card p-5">
              ...
              <p className="text-2xl font-bold">{formatCurrency(item.monthlyCost)}</p>
              <p className="mt-2 text-sm text-slate-500">Próximo cobro: {new Date(item.nextBillingDate).toLocaleDateString("es-ES")}</p>
              ...
              <div id={`edit-sub-${item.id}`} className="mt-5 rounded-2xl border border-slate-200 bg-slate-50 p-4">
        ...
        <div className="space-y-4">
          ...
          {recurringBills.map((item) => (
            <div key={item.id} className="card p-5">
              ...
              <div id={`edit-bill-${item.id}`} className="mt-5 rounded-2xl border border-slate-200 bg-slate-50 p-4">
